...
const main = async () => {
    const doctor = await prisma.user.findMany({ where: { role: "doctor" }, select: { id: true } })
    // console.info(doctor)
    const spesialis = ["umum",
        "gigi",
        "mata",
        "penyakit dalam",
        "kandungan",
    ...
    for (let i = 0; i < 5; i++) {
        await prisma.doctor.create({
            data: {
                ...
                price: parseInt(faker.commerce.price({ min: 100000, max: 500000, dec: 0 })),
